...
(async function () {
  ...
  await ncp(path.join(__dirname, `./template/`), name); //SET package.json

  let fileContent = fs.readFileSync(name + "/package.json", "utf8").toString();
  let template = fileContent.replace(new RegExp(`(\{\{PROJECT_NAME\}\}|\{\{ PROJECT_NAME \}\})`, "g"), '"' + name + '"');
  fs.writeFileSync(name + "/package.json", template); //SET .cracorc.js

  fileContent = fs.readFileSync(name + "/.cracorc.js", "utf8").toString();
